...
import DeleteHelpArticle from "./help-modal";
import HelpHeader from "./help-header";

class HelpPublished extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    render() {
        ...
        return (
            <>
                            ...
                            this.state.listByMe && this.state.listByMe.map((item, key) => {
                                return (
                                    <tr className="article-table" key={key}>
                                        <td className="txt-break title">
                                            <Link to={'/help-details/' + item._id}
                                                  style={{color: '#000', cursor: 'pointer', minWidth: '150px'}}>
                                                {item.title}
                                            </Link>
                                        </td>
                                        <td className="col-heavyDark txt-break">{localStorage.provider_name}</td>
                                        ...
                                        <td style={{textAlign: 'center'}}>
                                            <Link className="col-heavyDark view-delete"
                                                  to={'/help-details/' + item._id}>View</Link> |
                                            <Link className="col-buttonAndLink view-delete"
                                                  to={'/help-new/' + item._id}> Edit</Link> |
                                            <Link to={"#"} className="col-paragraphBg view-delete mouse-cursor"
                                               onClick={() => this.showModal(item._id)}> Delete</Link>
                                        </td>
                                    </tr>
                                )
                            })
                ...
                {/*  Modal  */}
                <DeleteHelpArticle show={this.state.show} deleteId={this.state.deleteId} handleClose={this.hideModal}/>
            </>
        )
    }
}
